...
//		Log.w("ReadEntryTask", "load :" + arg0[0]);
		...
		try {
			...
			if(tmp!=null&&tmp.trim().length()>0){
				index=tmp.indexOf("slideShow=");
				index2=tmp.indexOf(endTag);
				result=tmp.substring(index+10,index2+2);				
				try{
					 String thumb="";
					 String image="";
					 String id="";
					...
					for(int i=0;i<array.length();i++){
						...
						fullLink=obj.getString("fullscreenLink");
